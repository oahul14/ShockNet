...
from model.ShockNet import ShockSENet_3, ShockSENet_4
from model.TShockNet import TShockSENet_4
...
if __name__ == "__main__":
    ...
    description = """\
                    ...
                    Example: \n\
                        python train_model.py -s shocknet3 -m shocknet3_example.pth -e shocknet3_exp.txt -d nodal_48x96 -i KNI -l 0.0001 -b 14 14 -n 50 -w 0.01
                    """

    parser = argparse.ArgumentParser(description=description, formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument("--shocknet", "-s", type=str, narg=1, default="shocknet4",
                        help="model architecture: shocknet3, shocknet4 and tshocknet")
    parser.add_argument("--mname", "-m", type=str, narg=1, default="model.pth",
                        help="model name to be saved in .pth")
    parser.add_argument("--ename", "-e", type=str, narg=1, default="experiment.txt",
                        help="experiment text file name for saving experiments")  
    parser.add_argument("--dname", "-d", type=str, narg=1, default="nodal_48x96",
                        help="dataset to train with")  
    parser.add_argument("--init", "-i", type=str, narg=1, default="KNI",
                        help="initialisation method: KNI (norm) and KUI (uniform)")  
    parser.add_argument("--lr", "-l", type=float, narg=1, default=0.02,
                        help="learning rate value")
    parser.add_argument("--batch", "-b", type=int, narg=2, default=[14, 14],
                        help="training and validating batch sizes")
    parser.add_argument("--nepoch", "-n", type=int, narg=1, default=50,
                        help="epoch number")
    parser.add_argument("--weightdecay", "-w", type=int, narg=1, default=0,
